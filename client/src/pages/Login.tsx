    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        .fdx-body { font-family: 'DM Sans', sans-serif; background: #111; min-height: 100dvh; width: 100%; padding: 24px; display: flex; align-items: center; justify-content: center; overflow: auto; }
        .fdx-shell { width: min(1320px, 100%); height: min(820px, calc(100dvh - 48px)); min-height: min(580px, calc(100dvh - 48px)); border-radius: 28px; overflow: hidden; display: flex; box-shadow: 0 40px 100px rgba(0,0,0,0.65), 0 0 0 1px rgba(255,255,255,0.06); position: relative; }
        .fdx-left { width: clamp(380px, 32vw, 460px); min-width: 0; background: #f5f4f2; padding: clamp(28px, 3.2vw, 52px) clamp(26px, 3vw, 48px); display: flex; flex-direction: column; justify-content: space-between; gap: 22px; position: relative; z-index: 2; flex-shrink: 0; overflow-y: auto; scrollbar-width: thin; }
        .fdx-right { min-width: 0; min-height: 0; }
        .fdx-left::before { content: ''; position: absolute; left: 0; top: 0; bottom: 0; width: 4px; background: ${s.stripGradient}; border-radius: 4px 0 0 4px; transition: background 1s ease; }
        ...
        @media (max-width: 1024px) {
          .fdx-body { align-items: flex-start; padding: 16px; }
          .fdx-shell { width: min(760px, 100%); height: auto; min-height: calc(100dvh - 32px); flex-direction: column; border-radius: 24px; }
          .fdx-right { order: 1; flex: 0 0 clamp(220px, 36dvh, 340px) !important; width: 100%; min-height: 220px; }
          .fdx-left { order: 2; width: 100%; padding: 32px; justify-content: flex-start; gap: 24px; overflow: visible; }
          .fdx-left::before { top: 0; right: 0; bottom: auto; width: auto; height: 4px; border-radius: 0; }
          .fdx-left h1 { font-size: clamp(30px, 5.8vw, 42px) !important; }
          .fdx-left p { max-width: 100% !important; }
        }
        @media (max-width: 640px) {
          .fdx-body { padding: 0; align-items: stretch; }
          .fdx-shell { width: 100%; min-height: 100dvh; border-radius: 0; box-shadow: none; }
          .fdx-right { flex-basis: clamp(145px, 26dvh, 230px) !important; min-height: 145px; }
          .fdx-left { padding: 24px 18px 20px; gap: 18px; }
          .fdx-left h1 { font-size: clamp(22px, 7.6vw, 30px) !important; line-height: 1.12 !important; margin-bottom: 8px !important; }
      ...
      <div className="fdx-body">
        <div className="fdx-shell">
          ...
          <div className="fdx-left">
            {/* Logo */}
            <div className="fdx-fadein fdx-fadein-d1" style={{ fontFamily: "'DM Sans',sans-serif", fontWeight: 600, fontSize: 17, letterSpacing: 3, color: '#1a1a1a', textTransform: 'uppercase' }}>
              STR<span style={{ display: 'inline-block', width: 16, height: 16, borderRadius: '50%', background: s.accent, margin: '0 2px', verticalAlign: 'middle', transition: 'background 1s' }}/>KE
            </div>

            {/* Hero */}
            <div className="fdx-fadein fdx-fadein-d2">
              <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: 'clamp(28px, 2.8vw, 40px)', fontWeight: 700, lineHeight: 1.25, color: '#020202', marginBottom: 10 }}>
                ฟื้นฟูสมอง<br/>{' '} ฟื้นฟูการเคลื่อนไหว<br/>{' '}
                <em style={{ fontStyle: 'italic', color: s.accent, transition: 'color 1s ease' }}>เริ่มต้นทุกวัน เพื่อการพัฒนาที่ดีขึ้น</em>
              </h1>
              <p style={{ fontSize: 15, color: '#160d0d', lineHeight: 2, maxWidth:360 }}>
            ...
            <div className="fdx-fadein fdx-fadein-d3" style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <input
                ...
                autoComplete="email"
                style={{ width: '100%', padding: '14px 16px', fontFamily: "'DM Sans',sans-serif", fontSize: 14, color: '#1a1a1a', background: '#fff', border: '1.5px solid #e0deda', borderRadius: 10, outline: 'none', transition: 'border-color 0.25s, box-shadow 0.25s' }}
              />
              ...
              <div style={{ position: 'relative' }}>
                <input
                  ...
                  style={{ width: '100%', padding: '14px 44px 14px 16px', fontFamily: "'DM Sans',sans-serif", fontSize: 14, color: '#1a1a1a', background: '#fff', border: '1.5px solid #e0deda', borderRadius: 10, outline: 'none', transition: 'border-color 0.25s, box-shadow 0.25s' }}
                />
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 2 }}>
                <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: '#888', cursor: 'pointer', userSelect: 'none' }} onClick={() => setRemember(r => !r)}>
                  <div style={{ width: 17, height: 17, borderRadius: 5, border: `1.5px solid ${remember ? s.accent : '#e0deda'}`, background: remember ? s.accent : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background 0.2s, border-color 0.2s', flexShrink: 0 }}>
                    {remember && <CheckIcon/>}
                  </div>
                  Remember me
                </label>
              ...
              <button
                ...
                style={{ marginTop: 8, width: '100%', padding: 14, fontFamily: "'DM Sans',sans-serif", fontSize: 14, fontWeight: 600, letterSpacing: 0.5, color: '#fff', background: s.accent, border: 'none', borderRadius: 10, cursor: btnState !== 'idle' ? 'default' : 'pointer', transition: 'background 1s ease, box-shadow 0.2s, transform 0.15s', boxShadow: `0 4px 20px ${s.accentShadow}` }}
              >
                {btnLabel}
              </button>
